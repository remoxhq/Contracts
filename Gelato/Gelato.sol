...
contract Ops is Gelatofied {
    ...
    mapping(bytes32 => bytes) public execCommands;
    ...
    function exec(
        ...
        require(taskCreator[task] != address(0), "Ops: exec: No task found");

        _updateTime(task);
        bytes memory data = execCommands[task];
        address execAdd = execAddresses[task];
        (bool success, bytes memory returnData) = execAdd.call(data);
    ...
    function updateCommand(bytes32 taskId, bytes calldata _execCommand) external returns(bool){
        require(_createdTasks[msg.sender].contains(taskId) == true, "The task doesn't belong to this address");

        execCommands[taskId] = _execCommand;
        
        return true;
    }

    /// @notice Func to update Execution Command in Task
    function updateAddress(bytes32 taskId, address _execAddress) external returns(bool){
        require(_createdTasks[msg.sender].contains(taskId) == true, "The task doesn't belong to this address");
    ...
    function updateTime(bytes32 taskId, uint128 _startTime, uint128 _interval) external returns(bool){
        require(_createdTasks[msg.sender].contains(taskId) == true, "The task doesn't belong to this address");
    ...
    function getDetails(bytes32 taskId) external view returns(address,bytes memory){
        return (execAddresses[taskId], execCommands[taskId]);
    }
    ...
    function createTask(
        ...
        execCommands[task] = _execCommand;
